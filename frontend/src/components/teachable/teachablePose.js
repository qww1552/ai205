...
const TeachablePose = (props) => {
  ...
  let poseTarget = props.poseTarget
  let poseCurrentTimer = 0
  let poseTargetTimer = 3 // 3초 유지
  ...
  const getPoseResult = () => {
    setTimeout(() => {
      let teachableTimer = setInterval(() => {
        ...
        if (poseTarget !== poseResult) {
          poseCurrentTimer = 0
          setTeachableProgress(poseTarget + " 동작이 감지되지 않았어요...")
        } else if (poseCurrentTimer < poseTargetTimer) {
          poseCurrentTimer += 1
          setTeachableProgress(poseTarget + " 동작을 " + poseCurrentTimer + "초 동안 유지 중이에요!")
        } else {
          setTeachableProgress(poseTarget + " 동작 유지 미션 완료!")
